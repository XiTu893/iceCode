...
IceCode is an open-source AI coding agent CLI with desktop IDE integration.

Use OpenAI-compatible APIs, Gemini, GitHub Models, Codex OAuth, Ollama, and other supported backends while keeping one terminal-first workflow: prompts, tools, agents, MCP, slash commands, and streaming output. Includes a full-featured Electron-based desktop IDE.
...
If the install later reports `ripgrep not found`, install ripgrep system-wide and confirm `rg --version` works in the same terminal before starting IceCode.
