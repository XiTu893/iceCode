...
beforeEach(() => {
  ...
  testConfigDir = mkdtempSync(join(tmpdir(), 'IceCode-provider-config-'))
...
describe('setActiveProviderProfile', () => {
  test('sets OPENAI_MODEL env var when switching to an openai-type provider', async () => {
    const configDir = mkdtempSync(join(tmpdir(), 'IceCode-provider-config-'))
  ...
  test('persists no-key openai-compatible profiles for restart fallback', async () => {
    ...
    try {
      ...
      const persisted = JSON.parse(
        readFileSync(join(configDir, '.IceCode-profile.json'), 'utf8'),
      )

      expect(result?.id).toBe('ollama_prof')
      expect(existsSync(join(tempDir, '.IceCode-profile.json'))).toBe(false)
