...
const missingCodexAuthPath = join(tmpdir(), 'IceCode-missing-codex-auth.json')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'IceCode-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'IceCode-profile-file-'))
...
test('saveProfileFile defaults to user config instead of the working directory', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'IceCode-workspace-profile-'))
  const configRoot = mkdtempSync(join(tmpdir(), 'IceCode-config-profile-'))
...
test('loadProfileFile keeps project-local files as a legacy fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'IceCode-legacy-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'IceCode-empty-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('loadProfileFile does not fall back when user config profile is invalid', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'IceCode-invalid-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'IceCode-invalid-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('deleteProfileFile clears the default profile and legacy workspace fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'IceCode-delete-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'IceCode-delete-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('clearPersistedCodexOAuthProfile removes only persisted Codex OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'IceCode-codex-oauth-profile-'))
...
test('clearPersistedCodexOAuthProfile clears both default and legacy OAuth profiles', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'IceCode-clear-oauth-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'IceCode-clear-oauth-config-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('legacy openai saved profiles still deserialize and rebuild startup env', async () => {
  const tempDir = mkdtempSync(join(tmpdir(), 'IceCode-provider-'))
...
test('buildStartupEnvFromProfile preserves plural-profile env when the legacy file is stale', async () => {
  ...
  // addProviderProfile does NOT sync the legacy .IceCode-profile.json,
