...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.IceCode${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.IceCode${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.IceCode', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.IceCode', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.IceCode', 'skills')
