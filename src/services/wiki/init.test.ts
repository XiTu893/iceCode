...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'IceCode-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    join('.IceCode', 'wiki', 'schema.md'),
    join('.IceCode', 'wiki', 'index.md'),
    join('.IceCode', 'wiki', 'log.md'),
    join('.IceCode', 'wiki', 'pages', 'architecture.md'),
  ])
